"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
require("reflect-metadata");
const inversify_1 = require("inversify");
const api_controller_1 = __importDefault(require("./controllers/api.controller"));
const main_controller_1 = __importDefault(require("./controllers/main.controller"));
const user_controller_1 = __importDefault(require("./controllers/user.controller"));
const api_route_1 = __importDefault(require("./routes/api.route"));
const user_route_1 = __importDefault(require("./routes/user.route"));
const main_route_1 = __importDefault(require("./routes/main.route"));
const gift_route_1 = __importDefault(require("./routes/gift.route"));
const gift_controller_1 = __importDefault(require("./controllers/gift.controller"));
const game_route_1 = __importDefault(require("./routes/game.route"));
const game_controller_1 = __importDefault(require("./controllers/game.controller"));
const user_service_1 = __importDefault(require("./services/user.service"));
const game_service_1 = __importDefault(require("./services/game.service"));
const gift_service_1 = __importDefault(require("./services/gift.service"));
const game_user_gift_service_1 = __importDefault(require("./services/game-user-gift.service"));
const game_user_service_1 = __importDefault(require("./services/game-user.service"));
var DIContainer = new inversify_1.Container();
DIContainer.bind(main_controller_1.default).toSelf();
DIContainer.bind(api_controller_1.default).toSelf();
DIContainer.bind(user_controller_1.default).toSelf();
DIContainer.bind(gift_controller_1.default).toSelf();
DIContainer.bind(game_controller_1.default).toSelf();
DIContainer.bind("IRoute<IController>").to(main_route_1.default);
DIContainer.bind("IRoute<IController>").to(api_route_1.default);
DIContainer.bind("IRoute<IController>").to(user_route_1.default);
DIContainer.bind("IRoute<IController>").to(gift_route_1.default);
DIContainer.bind("IRoute<IController>").to(game_route_1.default);
DIContainer.bind(user_service_1.default).toSelf();
DIContainer.bind(game_service_1.default).toSelf();
DIContainer.bind(gift_service_1.default).toSelf();
DIContainer.bind(game_user_service_1.default).toSelf();
DIContainer.bind(game_user_gift_service_1.default).toSelf();
DIContainer.bind("IGameListener").to(game_user_gift_service_1.default);
DIContainer.bind("IUserListener").to(game_user_gift_service_1.default);
DIContainer.bind("IGiftListener").to(game_user_gift_service_1.default);
DIContainer.bind("IGameListener").to(game_user_service_1.default);
DIContainer.bind("IUserListener").to(game_user_service_1.default);
exports.default = DIContainer;
